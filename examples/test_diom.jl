...
# Solve Ax = b with DIOM and an ILU(0) preconditioner
yM = zeros(N)
yN = zeros(N)
yP = zeros(N)
...
x, stats = diom(A, b, M=opM, N=opN, verbose=true)
show(stats)
r = b - A * x
@printf("Residual norm: %8.1e\n", norm(r))

# Left preconditioning
x, stats = diom(A, b, M=opP, verbose=true)
show(stats)
r = b - A * x
@printf("Residual norm: %8.1e\n", norm(r))

# Right preconditioning
x, stats = diom(A, b, N=opP, verbose=true)
show(stats)
r = b - A * x
@printf("Residual norm: %8.1e\n", norm(r))
