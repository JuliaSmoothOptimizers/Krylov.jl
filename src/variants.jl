...
for fn in (:bicgstab!,)
  @eval begin
    $fn(solver :: BicgstabSolver{T,S}, A :: AbstractMatrix{T}, b :: SparseVector{T}; kwargs...) where {S, T <: AbstractFloat} =
      $fn(solver, PreallocatedLinearOperator(A, symmetric=true), convert(Vector{T}, b); wrap_preconditioners(kwargs, Vector{T})...)

    $fn(solver :: BicgstabSolver{T,S}, A :: AbstractMatrix{T}, b :: AbstractVector{T}; kwargs...) where {S, T <: AbstractFloat} =
