...
function symmlq!(solver :: SymmlqSolver{T,S}, A, b :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("SYMMLQ: system of size %d", n)
  ...
  (verbose > 0) && @info @sprintf("%5s  %7s  %7s  %8s  %8s  %7s  %7s  %7s", "Aprod", "‖r‖", "β", "cos", "sin", "‖A‖", "κ(A)", "test1")
  display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %8.1e  %8.1e  %7.1e  %7.1e", 0, rNorm, β, cold, sold, ANorm, Acond)
  ...
  while ! (solved || tired || ill_cond)
    ...
    display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %8.1e  %8.1e  %7.1e  %7.1e  %7.1e", iter, rNorm, β, c, s, ANorm, Acond, test1)
