...
function trimr!(solver :: TrimrSolver{T,S}, A, b :: AbstractVector{T}, c :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("TriMR: system of %d equations in %d variables", m+n, m+n)
  ...
  (verbose > 0) && @info @sprintf("%5s  %7s  %8s  %7s  %7s", "k", "‖rₖ‖", "αₖ", "βₖ₊₁", "γₖ₊₁")
  display(iter, verbose) && @info @sprintf("%5d  %7.1e  %8s  %7.1e  %7.1e", iter, rNorm, " ✗ ✗ ✗ ✗", βₖ, γₖ)
  ...
  while !(solved || tired)
    ...
    solved = rNorm ≤ ε
    tired = iter ≥ itmax
    display(iter, verbose) && @info @sprintf("%5d  %7.1e  %8.1e  %7.1e  %7.1e", iter, rNorm, αₖ, βₖ₊₁, γₖ₊₁)
  end
  (verbose > 0) && @printf("\n")
