...
function minres!(solver :: MinresSolver{T,S}, A, b :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("MINRES: system of size %d", n)
  ...
  (verbose > 0) && @info @sprintf("%5s  %7s  %7s  %7s  %8s  %8s  %7s  %7s  %7s  %7s", "Aprod", "‖r‖", "‖Aᵀr‖", "β", "cos", "sin", "‖A‖", "κ(A)", "test1", "test2")
  display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %7.1e  %8.1e  %8.1e  %7.1e  %7.1e", 0, rNorm, ArNorm, β, cs, sn, ANorm, Acond)
  ...
  while ! (solved || tired || ill_cond)
    ...
    display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %7.1e  %8.1e  %8.1e  %7.1e  %7.1e  %7.1e  %7.1e", iter, rNorm, ArNorm, β, cs, sn, ANorm, Acond, test1, test2)
