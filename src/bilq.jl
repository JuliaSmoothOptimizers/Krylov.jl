...
# Cahier du GERAD G-2019-42, GERAD, Montreal, 2019. doi:10.13140/RG.2.2.18287.59042
#
...
"""Solve the square linear system Ax = b using BiLQ method.

BiLQ algorithm is based on the Lanczos biorthogonalization process.
When A is symmetric and b = c, BiLQ is equivalent to SYMMLQ.

This version of BiLQ works in multiprecision.
"""
function bilq(A :: AbstractLinearOperator, b :: AbstractVector{T}; c :: AbstractVector{T}=b,
  ...
  x_lq = zeros(T, n)
  rNorm = bNorm = @knrm2(n, b) # ‖r₀‖
  rNorm == 0 && return (x_lq, zeros(T, n), SimpleStats(true, false, [rNorm], T[], "x = 0 is a zero-residual solution"))
  ...
  while !(solved || tired)
    ...
    if iter ≥ 2
      # Compute solution xₖ.
      # (xᴸ)ₖ₋₁ ← (xᴸ)ₖ₋₂ + ζₖ₋₁ * dₖ₋₁
      @kaxpy!(n, ζₖ₋₁ * cₖ,  d̅, x_lq)
      @kaxpy!(n, ζₖ₋₁ * sₖ, vₖ, x_lq)
    end
    ...
    if iter == 1
      ...
      ωₖ = βₖ₊₁ * sₖ * ζₖ₋₁
      rNorm_BiLQ = sqrt(μₖ^2 * norm_vₖ^2 + ωₖ^2 * norm_vₖ₊₁^2 + 2 * μₖ * ωₖ * dot_vₖ_vₖ₊₁)
    end

    # Compute BiCG residual norm
    # ‖rₖ‖ = |ρₖ| * ‖vₖ₊₁‖.
    ρₖ = βₖ₊₁ * (sₖ * ζₖ₋₁ - cₖ * ζbarₖ)
    rNorm_BiCG = abs(ρₖ) * norm_vₖ₊₁

    rNorm = min(rNorm_BiLQ, rNorm_BiCG)
    push!(rNorms, rNorm)
