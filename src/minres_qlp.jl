...
function minres_qlp!(solver :: MinresQlpSolver{T,S}, A, b :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("MINRES-QLP: system of size %d", n)
  ...
  ArNorms = T[]
  κ = zero(T)
  (verbose > 0) && @info @sprintf("%5s  %7s  %7s", "k", "‖rₖ‖", "‖Arₖ₋₁‖")
  ...
  while !(solved || tired || inconsistent)
    ...
    ζbarₖ = ζbarₖ₊₁
    βₖ = βₖ₊₁
    display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e", iter, rNorm, ArNorm)
  end
  (verbose > 0) && @printf("\n")
