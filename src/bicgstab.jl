...
function bicgstab!(solver :: BicgstabSolver{T,S}, A, b :: AbstractVector{T}; c :: AbstractVector{T}=b,
  ...
  (verbose > 0) && @info @sprintf("BICGSTAB: system of size %d", n)
  ...
  ε = atol + rtol * rNorm
  (verbose > 0) && @info @sprintf("%5s  %7s  %8s  %8s", "k", "‖rₖ‖", "αₖ", "ωₖ")
  display(iter, verbose) && @info @sprintf("%5d  %7.1e  %8.1e  %8.1e", iter, rNorm, α, ω)
