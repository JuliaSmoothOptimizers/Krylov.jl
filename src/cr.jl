...
function cr(A :: AbstractLinearOperator, b :: AbstractVector{T};
  ...
  descent = pr > 0.0 # pᵀr > 0 means p is a descent direction
  solved = rNorm ≤ ε
  tired = iter ≥ itmax
  on_boundary = false
  npcurv = false
    ...
    if radius > 0.0
      ...
      if abspAp ≤ γ * pNorm * @knrm2(n, q) # pᵀAp ≃ 0
        ...
        if abspr ≤ γ * pNorm * rNorm # pᵀr ≃ 0
          ...
          α = descent ? t1 : t2
          ρ > 0.0 && (tr = min(tr, rNorm² / ρ))
          Δ = -α * pr + tr * rNorm² - 0.5 * (tr)^2 * ρ # as pᵀAp = 0
          if Δ > 0.0 # direction r engenders a better decrease
      ...
      elseif pAp > 0.0 && ρ > 0.0 # no negative curvature
