...
@eval begin
  function usymlqr!(workspace :: UsymlqrWorkspace{T,FC,Sm,Sn}, $(def_args_usymlqr...); $(def_kwargs_usymlqr...)) where {T <: AbstractFloat, FC <: FloatOrComplex{T}, Sm <: AbstractVector{FC}, Sn <: AbstractVector{FC}}
    ...
    # Initialize orthogonal tridiagonalization process with respect to elliptic norms.
    ...
    while !(solved || tired || ill_cond || inconsistent || user_requested_exit || overtimed)
      ...
      # Continue the orthogonal tridiagonalization process with respect to elliptic norms.
      ...
      if ls && !solved_ls
        ...
        # tₖ = b - A * yₖ
        # println("Exact ‖rₖ‖ = ", norm(tₖ))
        # println("Exact ‖rₖ‖ = ", norm(rₖ))
        # println("Estimate ‖rₖ‖_M = ", norm(rNorm_ls))
      ...
      if ln && !solved_ln
        ...
        # println("Estimate ‖rₖ‖_LQ = ", norm(rNorm_ln))
