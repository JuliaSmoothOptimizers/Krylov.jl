...
krylov_dot(n :: Int, x :: Vector{T}, dx :: Int, y :: Vector{T}, dy :: Int) where T <: Number = dot(x, y)  # ignore dx, dy here

krylov_norm2(n :: Int, x :: Vector{T}, dx :: Int) where T <: BLAS.BlasReal = BLAS.nrm2(n, x, dx)
krylov_norm2(n :: Int, x :: Vector{T}, dx :: Int) where T <: Number = norm(x)  # ignore dx here

krylov_scal!(n :: Int, s :: T, x :: Vector{T}, dx :: Int) where T <: BLAS.BlasReal = BLAS.scal!(n, s, x, dx)
function krylov_scal!(n :: Int, s :: T, x :: Vector{T}, dx :: Int) where T <: Number
  @simd for i = 1:dx:n
    @inbounds x[i] *= s
  end
  return x
...
function krylov_axpy!(n :: Int, s :: T, x :: Vector{T}, dx :: Int, y :: Vector{T}, dy :: Int) where T <: Number
  # assume dx = dy
  @simd for i = 1:dx:n
    @inbounds y[i] += s * x[i]
  end
...
function krylov_axpy!(n :: Int, s :: T, x :: Vector{T}, dx :: Int, t :: T, y :: Vector{T}, dy :: Int) where T <: Number
  # assume dx = dy
  @simd for i = 1:dx:n
