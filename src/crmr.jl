...
function crmr!(solver :: CrmrSolver{T,S}, A, b :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("CRMR: system of %d equations in %d variables", m, n)
  ...
  display(iter, verbose) && @info @sprintf("%5d  %8.2e  %8.2e", 1, ArNorm, rNorm)
  ...
  while ! (solved || inconsistent || tired)
    ...
    iter = iter + 1
    display(iter, verbose) && @info @sprintf("%5d  %8.2e  %8.2e", 1 + 2 * iter, ArNorm, rNorm)
    solved = rNorm ≤ ɛ_c
    inconsistent = (rNorm > 100 * ɛ_c) && (ArNorm ≤ ɛ_i)
    tired = iter ≥ itmax
  end
