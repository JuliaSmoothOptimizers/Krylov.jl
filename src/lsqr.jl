...
function lsqr!(solver :: LsqrSolver{T,S}, A, b :: AbstractVector{T};
  ...
  (verbose > 0) && @info @sprintf("LSQR: system of %d equations in %d variables", m, n)
  ...
  (verbose > 0) && @info @sprintf("%5s  %7s  %7s  %7s  %7s  %7s  %7s  %7s  %7s", "Aprod", "α", "β", "‖r‖", "‖Aᵀr‖", "compat", "backwrd", "‖A‖", "κ(A)")
  display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e", 1, β₁, α, β₁, α, 0, 1, Anorm, Acond)
  ...
  while ! (solved || tired || ill_cond)
    ...
    display(iter, verbose) && @info @sprintf("%5d  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e  %7.1e", 1 + 2 * iter, α, β, rNorm, ArNorm, test1, test2, Anorm, Acond)
